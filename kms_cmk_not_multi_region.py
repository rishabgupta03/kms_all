#!/usr/bin/env python3
"""
Control: AWS KMS customer managed key is single-region.

Checks every KMS key in every enabled region and verifies that customer
managed keys (KeyManager == "CUSTOMER") are single-region keys
(MultiRegion == false), not multi-region keys.
...
def get_multi_region_role(detail):
    """
    Returns a human-readable note about the key's multi-region role
    (Primary or Replica), if applicable.
    """
    mr_config = detail.get("MultiRegionConfiguration")
    if not mr_config:
        return ""
    role = mr_config.get("MultiRegionKeyType", "Unknown")
    return f" (role: {role})"
...
def check_control(session):
    ...
    for region in tqdm(regions, desc="Scanning Regions"):
        ...
        for key in keys:
            ...
            total_checked += 1
            is_multi_region = detail.get("MultiRegion", False)

            if not is_multi_region:
                status = "COMPLIANT"
                compliant += 1
                evidence = "Key is single-region"
            else:
                status = "NON_COMPLIANT"
                non_compliant += 1
                role_note = get_multi_region_role(detail)
                evidence = f"Key is multi-region, expected single-region{role_note}"
...
def write_csv(results, account_id):
    ...
    filename = f"kms_cmk_single_region_{account_id}_{timestamp}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Check KMS customer managed keys are single-region (not multi-region)."
    )
    ...
    control_name = "KMS - Customer Managed Key Is Single-Region"
