#!/usr/bin/env python3
"""
Control: KMS customer managed key is enabled or scheduled for deletion.

Checks every KMS key in every enabled region and verifies that customer
managed keys (KeyManager == "CUSTOMER") are in one of two acceptable
states: "Enabled" (actively usable) or "PendingDeletion" (properly
scheduled for cleanup). A key left "Disabled" indefinitely - neither in
active use nor scheduled for removal - is considered non-compliant, since
it represents an unmanaged, forgotten resource.
...
ACCEPTABLE_STATES = {"Enabled", "PendingDeletion"}
...
def check_control(session):
    ...
    for region in tqdm(regions, desc="Scanning Regions"):
        ...
        for key in keys:
            ...
            if key_state in ACCEPTABLE_STATES:
                status = "COMPLIANT"
                compliant += 1
                evidence = f"Key state is '{key_state}'"
            else:
                status = "NON_COMPLIANT"
                non_compliant += 1
                evidence = (
                    f"Key state is '{key_state}', which is neither Enabled nor "
                    f"scheduled for deletion (PendingDeletion)"
                )
...
def write_csv(results, account_id):
    ...
    filename = f"kms_cmk_state_{account_id}_{timestamp}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Check KMS customer managed keys are Enabled or scheduled for deletion."
    )
    ...
    control_name = "KMS - Customer Managed Key Is Enabled or Scheduled for Deletion"
